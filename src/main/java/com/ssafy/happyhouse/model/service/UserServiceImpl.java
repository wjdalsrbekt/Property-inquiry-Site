...
public class UserServiceImpl implements UserService {
	...
	public List<MemberDto> userList(SearchDto searchDto) {
		...
		if(searchDto.getKey().equals("all")) {
			search.setKey("");
			search.setWord("");
		}else {
			search.setKey(searchDto.getKey());
			if(searchDto.getWord()=="" || searchDto.getWord()==null) {
				search.setKey("");
				search.setWord("");
			}else {
				search.setWord(searchDto.getWord());
			}
		}
		
		return sqlSession.getMapper(UserMapper.class).userList(searchDto);
	}
